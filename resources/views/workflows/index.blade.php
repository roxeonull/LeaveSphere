        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm overflow-hidden">
            <div class="flex items-center justify-between px-5 py-4 border-b border-gray-100 dark:border-gray-700">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <h3 class="font-semibold text-gray-900 dark:text-white text-sm">{{ $wf->name }}</h3>
                        <p class="text-xs text-gray-400">{{ $wf->department->name ?? 'All Departments' }} · {{ $wf->steps->count() }} approval levels</p>
                    </div>
                </div>
                <div class="flex items-center gap-2">
                    <button onclick="if(confirm('Delete this workflow?')) window.deleteWorkflow({{ $wf->id }})"
            ...
            <div class="px-5 py-4">
                ...
                <div class="flex items-center gap-2 flex-wrap">
                    @forelse($wf->steps as $i => $level)
                    <div class="flex items-center gap-2">
                        <div class="flex flex-col items-center">
                            <div class="w-8 h-8 {{ $colors[$i % count($colors)] }} rounded-full flex items-center justify-center text-white text-xs font-bold">
                                L{{ $level->level }}
                            </div>
                            <span class="text-xs text-gray-500 dark:text-gray-400 mt-1 whitespace-nowrap">{{ $level->approver_role }}</span>
                        </div>
                        @if(!$loop->last)
...
function workflowsPage() {
    return {
        ...
        async submitCreate() {
            ...
            try {
                const res = await fetch(`{{ route('workflows.store') }}`, {
                    method: 'POST',
                    headers: {
