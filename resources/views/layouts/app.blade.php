...
<head>
    ...
    <title>LeaveSphere - @yield('title', 'HR Analytics')</title>
        ...
        <nav class="flex-1 px-2 py-2 space-y-0.5 overflow-y-auto">
            ...
            @foreach($navItems as $item)
                <a href="{{ route($item['route']) }}"
                   class="sidebar-link flex items-center gap-3 px-3 py-2.5 rounded-lg text-gray-300 text-sm {{ request()->routeIs($item['route']) ? 'active text-white' : '' }}">
        ...
        <div class="border-t border-white/10 p-2 space-y-0.5">
            ...
            <div class="flex items-center gap-3 px-3 py-3 mt-1" x-show="!sidebarCollapsed">
                <div class="w-8 h-8 bg-blue-500 rounded-full flex items-center justify-center text-white text-xs font-bold flex-shrink-0">
                    {{ strtoupper(substr(auth()->user()->name ?? 'JA', 0, 2)) }}
                </div>
                <div class="min-w-0">
                    <p class="text-white text-xs font-medium truncate">{{ auth()->user()->name ?? 'John Admin' }}</p>
                    <p class="text-gray-400 text-xs truncate">{{ auth()->user()->role ?? 'Super Admin' }}</p>
                </div>
            </div>
        </div>
    </aside>
    ...
    <div class="flex-1 flex flex-col overflow-hidden">
        ...
        <header class="h-16 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between px-6 flex-shrink-0 shadow-sm">
            <div class="flex items-center gap-4">
                <h1 class="text-gray-800 dark:text-white font-semibold text-base">@yield('page-title', 'Dashboard')</h1>
            </div>
            <div class="flex items-center gap-3">
                ...
                <div class="relative" x-data="{ open: false }">
                    <button @click="open = !open"
                        ...
                        <span class="text-sm text-gray-700 dark:text-gray-300 hidden md:block">{{ auth()->user()->name ?? 'John Admin' }}</span>
                    ...
                    <div x-show="open" @click.away="open = false"
                        ...
                        <a href="{{ route('settings') }}" class="flex items-center gap-2 px-4 py-2 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700">
