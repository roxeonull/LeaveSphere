        ...
        <div class="flex items-center gap-2">
            ...
            <div class="relative" x-data="{ openExport: false }">
                ...
                <div x-show="openExport" @click.away="openExport = false"
                    ...
                    <a href="{{ route('users.export', 'excel') }}" class="flex items-center gap-2 px-4 py-2 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700">
                        <i data-lucide="table" class="w-4 h-4 text-green-600"></i> Excel
                    </a>
                    <a href="{{ route('users.export', 'pdf') }}" class="flex items-center gap-2 px-4 py-2 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700">
        ...
        <div class="stat-card bg-white dark:bg-gray-800 rounded-xl p-5 border border-gray-200 dark:border-gray-700 shadow-sm">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-3xl font-bold text-gray-900 dark:text-white mt-1">{{ $stats['managers'] }}</p>
                </div>
        ...
        <div class="stat-card bg-white dark:bg-gray-800 rounded-xl p-5 border border-gray-200 dark:border-gray-700 shadow-sm">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-3xl font-bold text-gray-900 dark:text-white mt-1">{{ $stats['employees'] }}</p>
                </div>
...
function usersPage() {
    return {
        ...
        async fetchData(page = 1) {
            ...
            try {
                const res = await fetch(`{{ route('users.data') }}?${params.toString()}`, {
        ...
        async submitForm() {
            ...
            const url = this.isEditing ? `/users/${this.editingId}` : `{{ route('users.store') }}`;
